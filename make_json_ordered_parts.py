...
order_date = pd.to_datetime(df_order["Order date"]).tolist()
delivery_date = pd.to_datetime(df_order["Desired delivery date"]).tolist()

def write_order_to_class():
    for part_id in ordered_part.part_id:
        indices_parts = [i for i, x in enumerate(order_part_id) if x == part_id]
        ...
        ordered_part.orders.total_quantity.append(sum([quantity[i] for i in indices_parts]))
...
def class_to_dict(obj):
    if hasattr(obj, "__dict__"):
        ...
        return obj.tolist()
    elif isinstance(obj, (datetime, pd.Timestamp)):   # <- handle both
